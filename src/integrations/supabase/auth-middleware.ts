...
export const requireSupabaseAuth = createMiddleware({ type: 'function' }).server(
  async ({ next }) => {
    ...
    const SUPABASE_URL = process.env.SUPABASE_URL || 'https://quvqjctlskmqeyhognvd.supabase.co';
    ...
    if (!SUPABASE_URL || !SUPABASE_PUBLISHABLE_KEY) {
      throw new Error('Missing Supabase URL or publishable key for server auth.');
    }
