-- RUN MANUALLY. Attendance eligibility must not depend on the att_session
-- still being 'open'. In the normal admin flow the session is CLOSED
-- (finalized) right before the exam starts, which previously locked out
-- every student marked present/late. This relaxes the gate so any recent
-- attendance record with status present/late qualifies, regardless of
-- whether the session is still open or has been closed/finalized.
...
BEGIN
  ...
  -- Attendance gate: an att_records row with status present or late
  -- in ANY att_session (open or closed). The finalize step legitimately
  -- flips the session to 'closed' right before the exam begins, so gating
  -- on 'open' locked everyone out. We scope by student email match to
  -- avoid granting access via a shared classSN alone.
  SELECT EXISTS (
    SELECT 1
      FROM public.att_records r
     WHERE r.status IN ('present', 'late')
